...
[**startSubscription**](SubscriptionsApi.md#startsubscription) | **POST** /customer/{customerId}/subscription/start | Start Subscription For Customer
...
# **startSubscription**
> \BillaBear\Model\Subscription startSubscription($body, $customer_id)
...
try {
    $result = $apiInstance->startSubscription($body, $customer_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling SubscriptionsApi->startSubscription: ', $e->getMessage(), PHP_EOL;
}
?>
```
