...
class ClientPaymentDetailCompleteFrontendTokenTest extends TestCase
{
    public function testCompleteFrontendTokenNotFound()
    {
        $this->expectException(NotFoundException::class);
        $requestSender = $this->createMock(RequestSenderInterface::class);
        $expected = ['id' => 'payment-details'];
        $response = new Response(404, $expected);
        $token = 'token-here';
        $requestSender->expects($this->once())->method('send')->with('POST', '/v1/customer/id-here/payment-methods/frontend-payment-token', ['token' => $token])->willReturn($response);

        $client = new Client($requestSender);
        $actual = $client->completeFrontendToken('id-here', $token);
    }

    public function testCompleteFrontendTokenUnexpectedResponse()
    {
    ...
    public function testCompleteFrontendToken()
    {
        $requestSender = $this->createMock(RequestSenderInterface::class);
        $expected = ['id' => 'payment-details'];
