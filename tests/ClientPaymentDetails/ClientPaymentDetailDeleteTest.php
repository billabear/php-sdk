...
class ClientPaymentDetailDeleteTest extends TestCase
{
    public function testFetchFrontendTokenNotFound()
    {
        ...
        $requestSender->expects($this->once())->method('send')->with('DELETE', '/v1/customer/id-here/payment-methods/second-id-here')->willReturn($response);

        $client = new Client($requestSender);
        $client->deletePaymentDetails('id-here', 'second-id-here');
    }
