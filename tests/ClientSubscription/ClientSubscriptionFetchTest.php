...
class ClientSubscriptionFetchTest extends TestCase
{
    public function testStartSubscriptionNotFound()
    {
        ...
        $requestSender->expects($this->once())->method('send')->with('GET', '/v1/subscription/subscription-id-here')->willReturn($response);

        $client = new Client($requestSender);
        $client->fetchSubscription('subscription-id-here');
    }
    ...
    public function testStartSubscriptionCreated()
    {
        ...
        $client = new Client($requestSender);
        $actual = $client->fetchSubscription('subscription-id-here');
